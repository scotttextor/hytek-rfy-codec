...
import { runActionDefsPass, isActionDefsPassEnabled, } from "./action-defs-pass.js";
...
export function generateFrameContextOps(frame, setup) {
    ...
    const diagonalCripples = allCripples.filter(sb => sb.role === "Kb" && (sb.box.xMax - sb.box.xMin) > KB_DIAGONAL_BBOX_X_MIN);
    ...
    for (const plate of plates) {
        ...
        for (const kb of diagonalCripples) {
            ...
            if (x1 === null || x2 === null)
                continue; // Kb doesn't reach plate
            const edge_lo = Math.min(x1, x2);
            const edge_hi = Math.max(x1, x2);
        ...
        const planNameForLbw7 = frame.planName ?? "";
        ...
        const isLbwLongTWithWBraces = isLbwFrame_lbw7 && plateIsT_lbw7 && isLongPlate_lbw7 &&
    ...
    const _planNameForNlbw = frame.planName ?? "";
    const isNlbwFrame = /(?:^|[-_/])(?:NLBW|NON-LBW)(?:[-_/]|$)/i.test(_planNameForNlbw);
    ...
    const subPanelNogNames = new Set();
    ...
    const subPanelNogEndpoints = new Map();
    if (isNlbwFrame && nogs.length >= 2) {
        ...
        if (longestNog) {
            ...
            for (const n of nogs) {
                if (Math.abs(n.box.cy - canonicalCy) <= 5)
                    continue; // canonical-row
    ...
    const raisedBPlates = layout.filter(sb => BOT_PLATE_ROLES.has(sb.role) && sb.box.cy > 30 && sb.horizontal);
    ...
    const crippleCompanionByStudName = new Map();
    for (let i = 0; i < studs.length; i++) {
        const a = studs[i];
        if (String(a.stick.usage ?? "").toLowerCase() !== "stud")
            continue;
        if (a.role !== "S")
            continue;
        ...
        for (let j = 0; j < studs.length; j++) {
            if (i === j)
                continue;
            const b = studs[j];
            if (String(b.stick.usage ?? "").toLowerCase() !== "stud")
                continue;
            if (b.role !== "S")
                continue;
            // Same bbox.cx (within 1mm) — paired vertically-stacked stud column.
            if (Math.abs(a.box.cx - b.box.cx) > 1)
                continue;
            ...
            if (a.stick.length <= b.stick.length)
                continue;
    ...
    if (fullHeightStuds.length >= 2 && wallSpanX > 0) {
        ...
        const _planLbw8A = frame.planName ?? "";
    ...
    const planNameForStuds = frame.planName ?? "";
    ...
    for (const stud of studs) {
        ...
        const isTrimStud = String(stud.stick.usage ?? "").toLowerCase() === "trimstud";
        ...
        //
        //   2026-05-08 attempt: added "OPPOSITE flipped" criterion thinking
        //   only lip-to-lip-box pairs got the 7-Web pattern. Result on HG260001
        //   PK1-current: still introduced 70 false-positive Web ops (L25/S6-S7,
        //   L34/S5-S6, L38/S9-S12, L12/S3-S4) with no parity gain (parity stayed
        //   at 72.83%, but extras count went from 3183 to 3253). Cohort win on
        //   HG260044 LBW (+0.79pp) didn't justify the false-positive regression
        //   on HG260001 PK1.
        //
        ...
        //   geometric pairing AND opposite-flipped check — likely tied to a
        //   structural-attachment marker in the XML we haven't identified
        //   (possibly differs between PK1 and PK4 of the same project).
        //
        //   Until that's understood, emitting NO Webs is safer than emitting
        //   wrong ones. This kept-disabled choice is preferred by the principal
        //   "no regression on others" constraint when the false-positive risk
        //   varies across the customer corpus.
        ...
        for (const nog of nogs) {
            ...
            const useSwage = (!isTrimStud || isInteriorSubPanelTrimStud) && (lipNeighbor ||
                _lbw8CcPin ||
                (!isWallEndStud) ||
        ...
        for (const kb of diagonalCripples) {
            ...
            if (range < 50)
                continue; // not a Kb-mid-wall connection
            ...
            if (studLocalLo < 0 || studLocalHi > stud.stick.length)
                continue;
            // Skip near stud caps (already handled by per-stick rules).
            if (studLocalLo < 50 || studLocalHi > stud.stick.length - 50)
                continue;
            ...
            if (kbStart && kbEnd) {
                ...
                if (kbEnd.z > kbStart.z)
                    plateSideShift = +KB_NOTCH_SHIFT; // Kb plate at top → shift up
                else
                    plateSideShift = -KB_NOTCH_SHIFT; // Kb plate at bottom → shift down
            }
    ...
    for (const header of headers) {
        ...
        const crossingAngles = new Map(); // localPos → angleDeg from horizontal
        ...
        const planNameForBodyPair = frame.planName ?? "";
        ...
        for (let i = 0; i < crossings.length; i++) {
            ...
            if (cluster.length >= 2) {
                ...
                const useAngleForCluster = useAngleAwarePair && (allFromWBrace || (cluster.length >= 3 && wAnglesInCluster.length >= 2));
    ...
    // ---------------------------------------------------------------------
    const frameTypeForMh = String(frame.frameType ?? "").toLowerCase();
    if (frameTypeForMh === "ceilingpanel") {
        ...
        const bPlatesInFrame = layout.filter(sb => BOT_PLATE_ROLES.has(sb.role));
        ...
        if (bPlatesInFrame.length >= 2) {
            ...
            for (const p of bPlatesInFrame) {
                if (p.box.xMin < frameXMin)
                    frameXMin = p.box.xMin;
                if (p.box.xMax > frameXMax)
                    frameXMax = p.box.xMax;
            }
            const frameXSpan = frameXMax - frameXMin;
            // Real-stud x-positions for endpoint classification.
            const studXs = studs.map(s => s.box.cx).sort((a, b) => a - b);
            const bClassifications = [];
            for (const p of bPlatesInFrame) {
            ...
            result.__mhClassifications =
                new Map(bClassifications.map(c => [c.name, c.klass]));
 ...
 */
export function joinAdjacentSpannedOps(stickOps, opType, gap) {
