...
export function synthesizeRfyFromPlans(project, options = {}) {
    ...
    for (const plan of project.plans) {
        ...
        for (const frame of plan.frames) {
            ...
            try {
                ...
                if (coerced) {
                    try {
                        ...
                        console.warn(`Frame "${frame.name}": 4-vertex envelope coerced to bounding rectangle ` +
                            `(original ${e.message})`);
                    }
                    catch (e2) {
                        throw new Error(`Frame "${frame.name}": ${e.message} ` +
                            `(coerce also failed: ${e2.message})`);
                    }
                }
                else {
                    throw new Error(`Frame "${frame.name}": ${e.message}`);
            ...
            // detection logic on the projected geometry. This adds LIP NOTCH +
            // INNER DIMPLE at every stud-to-plate intersection, SWAGE + INNER DIMPLE
            // at every nog-to-stud crossing, etc. Without this, sticks have only
            // end-anchored tooling and are missing 60-80% of their cuts.
            //
            // Steps:
            //   1. Project every stick's start/end into frame-local 2D
            //   2. Compute outline corners (rectangle around the diagonal axis)
            //   3. Build a synthetic RfyFrame so generateFrameContextOps can run
            ...
            const mhBClassifications = contextOps.__mhClassifications;
            ...
            for (const stick of frame.sticks) {
                stickCount++;
                const isTb2bTrussMember = isTb2bTrussFrame &&
                ...
                const tinHnPanelApplied = stick[HN_PANELPOINT_APPLIED_KEY] === true;
                const ctx = isTb2bTrussMember || tinHnPanelApplied
                    ? []
                ...
                if (isRpPlanName(plan.name)
                    ...
                    if (len2D > 1e-3) {
                        ...
                        scaleRpDiagonalTplateBodyOps({ name: stick.name, length: len3D, outlineCorners: corners, tooling: merged }, plan.name);
                    }
                }
                // RP single-T-plate rake cap rewrite (Agent RP5, 2026-05-10).
                if ((typeof process === "undefined" || process.env?.CODEC_DISABLE_RP_ASC !== "1")
                    && stick.name === "T1"
                    && /^T\d/.test(stick.name)
                    && String(stick.usage ?? "").toLowerCase() === "topplate") {
                    const rakeDir = rpRakeDirectionForFrame(frame, plan.name);
                    if (rakeDir !== null) {
                        const lenRake = Math.sqrt((stick.end.x - stick.start.x) ** 2 +
                            (stick.end.y - stick.start.y) ** 2 +
                            (stick.end.z - stick.start.z) ** 2);
...
// ---------------------------------------------------------------------------
// Frame-context tooling integration
// ---------------------------------------------------------------------------
/**
 * Run the codec's frame-context crossing detector on a ParsedFrame.
 *
 * For each stick in the frame:
 *   1. Project to 2D using the frame's basis
 *   2. Build outline corners (4-point rectangle around the diagonal axis,
 *      thickness perpendicular = max of l/r flange)
 * Then construct a synthetic RfyFrame and call generateFrameContextOps,
 * which returns a Map of stick-name → tooling ops generated by crossings.
 *
...
function computeFrameContextOps(frame, basis, setup, planName) {
    ...
    for (const stick of frame.sticks) {
        ...
        if (len2D < 1e-6)
            continue;
        const dirX = dx / len2D, dirY = dy / len2D;
        ...
        const profile = {
            ...
            lip: stick.profile.lLip || stick.profile.rLip || 12,
        };
        syntheticSticks.push({
            name: stick.name,
            length: len3D,
            type: stick.usage.toLowerCase() === "topplate" || stick.usage.toLowerCase() === "bottomplate" ? "plate" : "stud",
    ...
    if (frame.type)
        syntheticFrame.frameType = frame.type;
    try {
 ...
 */
function computeNonPrimaryBPlateNames(frame, planName) {
    const out = new Set();
    // Only LBW/NLBW plans (slab-anchored wall types).
    if (!/-(LBW|NLBW)-/i.test(planName))
        return out;
    // Skip upper floors — slab anchors only on ground floor.
    if (/-(1F|2F|3F)-/i.test(planName))
        return out;
    if (!frame.sticks || frame.sticks.length === 0)
        return out;
    const bPlates = [];
    for (const s of frame.sticks) {
        if (!/^B\d/.test(s.name))
            continue;
        if (/^Bh/.test(s.name))
            continue; // raised B-plates handled separately
        const u = (s.usage ?? "").toLowerCase();
        if (u === "bottomchord")
            continue; // truss chords aren't slab plates
    ...
    for (const p of bPlates) {
        if (p.midZ - lowestZ > SUBPLATE_Z_THRESHOLD)
            out.add(p.name);
    }
    return out;
}
 ...
 */
function stripSlabAnchorOps(ops) {
    return ops.filter(op => {
        if (op.kind !== "point")
            return true;
        // Web @8 (slab-attach Web) at the start. Use a small tolerance.
        if (op.type === "Web" && Math.abs(op.pos - 8) < 0.5)
            return false;
...
/**
 * Merge per-stick tooling (from the rules engine) with frame-context tooling
 * (from crossings). Sort by position so the output XML is in physical-stick
 ...
 * 50mm+ from the ends).
 */
function applyMhBPlateOpsTransform(tooling, klass) {
    let stickLen = 0;
    for (const op of tooling) {
        if (op.kind === "spanned" && op.endPos > stickLen)
            stickLen = op.endPos;
    ...
    if (klass === "header") {
        ...
        for (const op of tooling) {
            ...
            if (!isStartCap && !isEndCap)
                continue;
            const already = existingInner.some(x => Math.abs(x.s - op.startPos) < SPAN_MATCH_TOL
                && Math.abs(x.e - op.endPos) < SPAN_MATCH_TOL);
            if (already)
                continue;
            toAdd.push({ s: op.startPos, e: op.endPos });
