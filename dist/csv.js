...
function toolingToCsvCells(tooling, stickLength) {
        ...
        switch (op.kind) {
            ...
            case "spanned": {
                ...
                for (const pos of positions)
                    flat.push({ type: op.type, pos, edgePriority: 1, bodySubPriority: 0, insIdx: counter++ });
                break;
            }
        }
    }
    flat.sort((a, b) => {
        if (a.pos !== b.pos)
            return a.pos - b.pos;
        if (a.edgePriority !== b.edgePriority)
            return a.edgePriority - b.edgePriority;
        ...
        if (a.bodySubPriority !== b.bodySubPriority)
            return a.bodySubPriority - b.bodySubPriority;
 ...
 */
function shouldEmitStartChamfer(cls, isTrussPlan, isFirstInBlock, stickLength, minKbLengthInFrame) {
    if (isTrussPlan)
        return true;
    if (cls === "W")
        return isFirstInBlock;
    if (cls === "Kb")
        return Math.abs(stickLength - minKbLengthInFrame) < 1;
    return false;
}
/**
 ...
 */
function filterToolingForCsv(tooling, emitStartChamfer) {
    if (emitStartChamfer)
        return tooling;
