...
export function runActionDefsPass(layout, config) {
    ...
    if (process.env.CODEC_ACTION_DEFS_DEBUG === "1") {
        ...
        const allClassCounts = new Map();
        for (const cr of crossings) {
            const className = classifyJoint(deriveStickProps(cr.connector, config.setup), deriveStickProps(cr.connectee, config.setup), flags);
            allClassCounts.set(className, (allClassCounts.get(className) ?? 0) + 1);
        }
        console.error(`[action-defs] plan=${planLabel} crossings=${crossings.length} ` +
