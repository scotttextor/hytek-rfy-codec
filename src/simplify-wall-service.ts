 ...
 *  Pattern-A formula. */
function isKbStick(stick: ParsedStick): boolean {
  if (!/^Kb\d/.test(stick.name)) return false;
...
function applicableKbZLinePositions(
  stick: ParsedStick,
  serviceActions: ReadonlyArray<ServiceAction>,
  length: number,
  extra: number,
  ...
  if (sinTheta < 0.1) return []; // near-horizontal Kb — no IS

  const stickPerpAxis: "x" | "y" = Math.abs(dxk) > Math.abs(dyk) ? "y" : "x";
  const stickRunAxis: "x" | "y" = stickPerpAxis === "y" ? "x" : "y";
  ...
  for (const svc of serviceActions) {
    const svcDz = Math.abs(svc.start.z - svc.end.z);
    if (svcDz > 0.01) continue; // only horizontal Service lines (V handled separately)
    const z_h = svc.start.z;
    if (z_h < zMin - 0.5 || z_h > zMax + 0.5) continue;
    const svcDx = Math.abs(svc.end.x - svc.start.x);
    const svcDy = Math.abs(svc.end.y - svc.start.y);
    const svcAxis: "x" | "y" = svcDx >= svcDy ? "x" : "y";
    ...
    if (stickRunHi < svcLo - 5 || stickRunLo > svcHi + 5) continue;
  ...
  for (const p of positions) {
    if (out.length > 0 && Math.abs(out[out.length - 1]! - p) < 1.5) continue;
    out.push(p);
  }
  return out;
}
...
function applicableKbVLineTopProjections(
  stick: ParsedStick,
  serviceActions: ReadonlyArray<ServiceAction>,
  length: number,
): number[] {
  const sStart = stick.start, sEnd = stick.end;
  if (sEnd.z <= sStart.z) return []; // top-attached only
  const dxk = sEnd.x - sStart.x, dyk = sEnd.y - sStart.y, dzk = sEnd.z - sStart.z;
  const lenK = Math.sqrt(dxk * dxk + dyk * dyk + dzk * dzk);
  if (lenK < 1) return [];
  const sinTheta = Math.abs(dzk) / lenK;
  if (sinTheta < 0.1) return [];
 ...
 *  (no-op here for Kbs). */
export function simplifyWallServiceFrame(frame: ParsedFrame, projectConfig?: ProjectConfig): void {
  ...
  for (const stick of frame.sticks) {
    ...
    if (kbExtra > 0 && isKbStick(stick)) {
      ...
      stripInnerServicePointOps(stick.tooling);
      const seen: number[] = [];
      const seenAdd = (p: number) => {
        for (const s of seen) if (Math.abs(s - p) < 1.5) return false;
        seen.push(p);
        return true;
      };
      for (const p of hPositions) {
        if (!seenAdd(p)) continue;
        stick.tooling.push({ kind: "point", type: "InnerService", pos: p });
      }
