...
function applicableKbZLinePositions(
  ...
  for (const svc of serviceActions) {
    const svcDz = Math.abs(svc.start.z - svc.end.z);
    if (svcDz > 0.01) continue; // only horizontal Service lines (V handled separately, deferred)
    const z_h = svc.start.z;
...
function applicableKbVLineTopProjections(
  ...
  for (const svc of serviceActions) {
    ...
    if (svcDz < 0.01) continue;
    if (svcDx > 0.01 || svcDy > 0.01) continue;
    // Same wall plane.
    ...
    if (worldZAtIS >= zPlate) continue; // out of Kb range
