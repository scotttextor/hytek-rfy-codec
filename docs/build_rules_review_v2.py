...
# same frame/stick.
# Rules whose answer was found in HYTEK config / docs / Detailer User Manual (move from "No" -> "Spec'd").
# Each entry: (source_file_or_concept, what to implement in the codec).
SPEC_FOUND = {
    5: ("Detailer User Manual p.97 (Service Hole Heights) + p.107 (Place Service Hole Route) + docs/service-z-line-design.md",
        "PER-FRAME setting in Detailer. 'Service Hole (Heights)' — heights are per-frame (296/446 in HYTEK practice). 'Place Service Hole Route' — operator draws a line through frame; service hole emitted wherever line crosses a stud. Read <tool_action name=Service> elements per frame from XML. For each horizontal z-line, project onto each stud. Emit InnerService at local_pos = z_line - z_stud_start - 2mm."),
    6: ("Detailer User Manual p.97 + docs/service-z-line-design.md",
        "Same per-frame z-line as rule 5 — when 2 service-hole heights are configured (typically 296+446), the route line crosses each stud at both heights."),
    7: ("Detailer User Manual: Bottom Plate Bolt Holes / Triple Web Hole Spacing settings",
        "Web access holes are emitted by the per-frame 'Web Hole' tool action, NOT by a fixed-spacing rule. Likely tied to triple-web-hole spacing setting (Triple Web Hole Spacing field)."),
    9: ("Detailer User Manual p.32 (Chamfer Tolerance) + machine-setups.ts: chamferTolerance + automaticChamfer",
        "MANUAL: 'Chamfer Tolerance — When one member intersects another at an angle (e.g. a brace), this is the distance that the corner of that member comes within the web side of the intersecting member before a chamfer is applied. 2mm is generally sufficient for 0.75 to 0.95 material as this also allows for the radius in the folds. Increase this figure for thicker material.' HYTEK setup uses 4mm. RULE: per-end geometric collision check; chamfer fires when corner protrudes into adjacent member's web by more than chamferTolerance."),
    12: ("Detailer User Manual p.32 + machine-setups.ts: chamferDetail + chamferTolerance",
        "Replace '45/cos(angle)' formula with cap span derived from chamfer triangle's max-x (14.5mm) projected onto the angled stick axis. Once auto-chamfer cuts the corner, the end-cap follows the cut edge geometrically."),
    14: ("Detailer User Manual p.32 + machine-setups.ts (89mm setup)",
        "Same auto-chamfer rule as rule 9, applied to 89mm Kb. Setup 6 (F325iT 89mm) has identical chamfer constants to setup 2 (70mm)."),
    29: ("Detailer User Manual p.97 (Service Hole) — toggle applies to plates too",
        "Same z-line projection rule applied to top plates. Existing post-decode block at scripts/diff-vs-detailer.mjs:642 already does this for T+N nogs — extend to all wall-plan T plates."),
    32: ("Detailer User Manual p.97 (Bottom Plate Bolt Holes)",
        "MANUAL: 'Bottom Plate Bolt Holes — Turns bottom plate bolt holes on or off. If your machine doesn't have a bolt hole, but has a web hole, a web hole will be placed instead of a bolt hole.' This is a PER-FRAME toggle. Wall frames have it ON; truss/joist frames have it OFF. Web@8 vs Bolt@62 are the same op served by different tools (machine-dependent)."),
    34: ("Detailer User Manual p.97 (Bottom Plate Bolt Holes)",
        "Same per-frame toggle as rule 32. Bolt position = setup.boltHoleToEnd (= 20 in setup, but HYTEK uses 62 — likely a HYTEK construction-detail override applied at frame creation). Per-frame bolt placement settings live in the frame's XML attributes."),
    36: ("Detailer User Manual p.97 (Bottom Plate Bolt Holes)",
        "Mirror of rule 34 at the end of the plate."),
    38: ("Detailer User Manual p.97 + machine-setups.ts: gauge-conditional behaviour",
        "Same toggle as rule 32. The gauge<1.0 gate may come from a Section Setup override on thicker steel."),
    42: ("Detailer User Manual p.97 (Bottom Plate Bolt Holes)",
        "Same per-frame toggle as rule 32 — applies to raised B plates too. NLBW vs LBW frame-type difference: NLBW frames have it set to ON for the rough-opening sill (the raised Bh acts as a slab plate); LBW raised Bh is structurally tied through the king-stud chain instead."),
    43: ("Detailer User Manual p.97",
        "Mirror of rule 42 at the end."),
    50: ("machine-setups.ts: setup 6 (89mm) constants",
        "Confirmed mirror of 70mm rule. Empirical match 90.3% across 1885 89mm nogs. Already implemented."),
    54: ("Detailer User Manual p.100 (Head Split) + Triple Stud Connections (p.101)",
        "PAIRED dimple at 58.5 corresponds to a TRIPLE STUD CONNECTION fixing point on a load-bearing header. 'Head Split' divides the head horizontally — the split header has 2 fixing points (16.5 + 58.5). Empirical: 71.5% LBW vs 1.7% NLBW."),
    55: ("Detailer User Manual p.100",
        "Mirror of rule 54 at the end."),
    59: ("Detailer User Manual p.100 (Head Split + Head Spacing) + Boxing tab p.34",
        "MANUAL: 'Head Split — Option to divide the head horizontally.' A 'paired header' = a Head-Split header (boxed). Web stiffener holes appear on the boxed pieces. Detection: frame has Head Split = ON. The H1/H3 detection in our codec is approximating this."),
    60: ("Detailer User Manual p.100",
        "89mm headers may default to Head Split = ON (heavier section needs more capacity). Verify against 89mm NLBW headers in corpus."),
    61: ("Detailer User Manual p.32 + machine-setups.ts",
        "Same auto-chamfer geometric check as rule 9. Replace 'angle >= 28 deg' threshold with collision detection (chamferTolerance = 2-4mm)."),
    64: ("Detailer User Manual p.32 + machine-setups.ts: chamferDetail",
        "Replace '39/cos(a) + 8*tan^2(a)' empirical curve with cap-span derived from the chamfered cut edge geometry."),
    66: ("Detailer User Manual p.32",
        "Same as rule 61."),
    74: ("Detailer User Manual p.100-101 (Sill Split, similar to Head Split)",
        "Sills (L role) are subject to 'Sill Split' — analogous to Head Split. The 58.5 paired dimple appears on split sills."),
    75: ("Detailer User Manual p.101",
        "Mirror of rule 74."),
    79: ("Detailer User Manual p.29 (Extra Fastenings — used for webbed joists)",
        "MANUAL: 'Extra Fastenings — Places extra flange holes adjacent to dimples for double screwing members. Generally used for webbed joists. Only applicable if machine has flange hole tool available.' Br/R sticks may be webbed-joist-type members. Extra Fastening Distance = 9mm. The 11mm dimple offset may relate to this."),
    80: ("Detailer User Manual p.29",
        "Same as rule 79 — Extra Fastening Distance (9mm) plus tolerance gives ~11mm."),
    81: ("Detailer User Manual p.29",
        "Same as rule 79 — mirror at end."),
    82: ("Detailer User Manual p.29",
        "Same as rule 80 — mirror at end."),
    86: ("fc-dat-rules.ts: cenCenWbHole + bcDistCenHole + webIncrements + horizChdCenHole (per-profile) + Detailer User Manual: Auto Build / truss scripts",
        "Implement panel-point detector: for each Web stick endpoint, project onto the chord centerline. Emit paired InnerDimples at +/- bcDistCenHole (50mm for 70mm normal-axis, 45mm for 89mm) and a LipNotch perpendicular to the chord. Suppress if two panel-points are within cenCenWbHole=120mm. Empirical: 94.9% on TIN, 100% on FJ, 5.8% on TB2B (TB2B uses different rule)."),
    89: ("docs/tb2b-patch-audit.md (14 patches) + Detailer User Manual: Boxing tab p.34",
        "Port 14 patches from scripts/diff-vs-detailer.mjs to src/. Boxing options (Boxed End Length 70mm, Boxing Max Dimple Spacing 1200mm, Boxing Offset) drive TB2B-specific tooling."),
    90: ("fc-dat-rules.ts: profile-2 (Linear) constants",
        "Linear-truss profile has shortenDblesWb=-50, endWbSetback=50, kpTruncated=90 (vs 0 for STD). Rewire src/simplify-linear-truss.ts to read from fc-dat-rules.ts."),
    91: ("Detailer User Manual p.97 + docs/service-z-line-design.md",
        "Same as rules 5/6 — replace blind emission with per-stud z-line projection. Read configured Service Hole Heights per frame; project onto each stud."),
}
...
OUT = os.path.join(ROOT, "docs", "rules-review-v5.xlsx")
wb.save(OUT)
print(f"Wrote: {OUT}")
