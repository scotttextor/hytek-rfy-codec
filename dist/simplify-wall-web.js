 ...
 *  case-insensitively. */
export function isWallWebPlanName(planName) {
...
export function applicableWebPositionsForStud(stick, webActions, length) {
...
function appendDedupedWebOps(tooling, positions, length) {
    const existingWebPositions = [];
    for (const op of tooling) {
        if (op.kind === "point" && op.type === "Web")
            existingWebPositions.push(op.pos);
    }
    for (const p of positions) {
        if (existingWebPositions.some(ep => Math.abs(ep - p) < 1))
            continue;
...
export function simplifyWallWebFrame(frame) {
    ...
    for (const stick of frame.sticks) {
        ...
        if (positions.length > 0)
            appendDedupedWebOps(stick.tooling, positions, length);
    }
}
...
export function simplifyWallWebInProject(plans) {
    for (const plan of plans) {
        if (!isWallWebPlanName(plan.name))
            continue;
