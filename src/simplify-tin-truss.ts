...
function fixTinTrussDiagonalDimplePosition(stick: ParsedStick): number {
  if ((stick.usage ?? "").toLowerCase() !== "web") return 0;
  if (!/^W\d/.test(stick.name)) return 0;
  const dx = stick.end.x - stick.start.x;
  const dy = stick.end.y - stick.start.y;
  ...
  for (const stick of frame.sticks) {
    ...
    if (isDiagonalWeb(stick)) {
      ...
      if (len >= 620 && len <= 800 && angle >= 40) {
        ...
        if (removed > 0) diagonalsChamferStripped.push(stick.name);
      }
...
function isQualifyingHnFrame(frame: ParsedFrame): boolean {
  if (!/^HN\d+-\d+$/.test(frame.name)) return false;
  // Frame envelope width: max(|V0-V1|, |V1-V2|) gives the in-plane width.
  if (!frame.envelope || frame.envelope.length < 4) return false;
  ...
  if (width < HN_MIN_WIDTH_MM) return false;
...
function emitTinHnScrewHoles(frame: ParsedFrame): number {
  ...
  const bottomChords = frame.sticks.filter(
    s => (s.usage ?? "").toLowerCase() === "bottomchord",
  );
  if (bottomChords.length === 0) return 0;
  ...
  for (const stick of frame.sticks) {
    ...
    if (usage === "bottomchord") {
      ...
      const hasStartScrew = stick.tooling.some(
        op => op.kind === "point" && op.type === "ScrewHoles" && op.pos < 50,
      );
      if (!hasStartScrew) {
        stick.tooling.push({
          kind: "point",
    ...
    if (!/^W\d/.test(stick.name) || usage !== "web") continue;
    ...
    if (!inHeelCohort) continue;

    // Skip if there's already a ScrewHoles on this stick (defensive).
    if (stick.tooling.some(op => op.kind === "point" && op.type === "ScrewHoles")) continue;
    ...
    if (horiz >= VERTICAL_HORIZ_TOL_MM) {
      ...
      if (len < 300) endOffset = SH_VERTICAL_NEAR_END_MM;
      else if (len < 1000) endOffset = SH_VERTICAL_FAR_END_MM;
      else endOffset = SH_VERTICAL_NEAR_END_MM;
...
interface ChordCrossing {
  name: string;
  tin: number;          // chord-local position from chord.start
  perp: number;         // perpendicular distance from web endpoint to chord centerline
}
...
function findChordCrossings(chord: ParsedStick, webs: ParsedStick[]): ChordCrossing[] {
  const chDir = {
  ...
  for (const w of webs) {
    ...
    if (tin < -10 || tin > chLen + 10) continue;
...
function groupPanelPairs(
  crossings: ChordCrossing[],
): Array<[ChordCrossing, ChordCrossing | null]> {
  const pairs: Array<[ChordCrossing, ChordCrossing | null]> = [];
  ...
  for (let i = 0; i < crossings.length; i++) {
    ...
    for (let j = i + 1; j < crossings.length; j++) {
      ...
      if (crossings[j]!.tin - a.tin > HN_PANELPOINT_MAX_PAIR_DELTA_MM) break;
      pi = j;
      break;
    }
    if (pi >= 0) {
...
function emitHnTopChordPanelPattern(chord: ParsedStick, webs: ParsedStick[]): number {
...
function emitHnPanelPatternsForFrame(frame: ParsedFrame): number {
  if (!isQualifyingHnFrame(frame)) return 0;
