...
import type { RfyToolingOp, RfyFrame, RfyStick, RfyProfile } from "./format.js";
import {
  type MachineSetup,
...
export interface ParsedStickProfile {
  web: number;        // mm — web depth (long C dimension)
  lFlange: number;    // mm — left flange (varies asymmetric C)
  rFlange: number;    // mm — right flange
  lLip: number;
  rLip: number;
  shape: string;      // "C", "S", etc.
  gauge: string;      // "0.75"
}

export interface ParsedStick {
  name: string;                 // "S1", "T1", "Kb1"
  start: Vec3;                  // world 3D
  end: Vec3;                    // world 3D
  flipped: boolean;
  profile: ParsedStickProfile;
  usage: string;                // raw input XML attribute: "TopPlate", "Stud", "Nog", etc.
  tooling: RfyToolingOp[];      // pre-generated by caller (rules engine)
...
export interface ParsedPlan {
  name: string;        // "GF-LBW-70.075"
  frames: ParsedFrame[];
}

export interface ParsedProject {
  name: string;        // "HG260001 LOT 289 COORA CRESCENT CURRIMUNDI 4551"
  jobNum: string;
  client: string;
...
// ---------------------------------------------------------------------------
// Frame basis (envelope → local 2D coordinate system)
// ---------------------------------------------------------------------------

export interface FrameBasis {
  origin: Vec3;        // V0 (frame-local origin in world coords)
  right: Vec3;         // unit vector along V1-V0 (frame-local +X)
  up: Vec3;            // unit vector orthogonal to right, in plane of envelope (frame-local +Y)
  normal: Vec3;        // right × up — out of envelope plane
  width: number;       // |V1 - V0| (frame "length" in elevation-x)
  height: number;      // |V3 - V0| projected onto up axis (frame "height" in elevation-y)
}
 ...
 */
export function deriveFrameBasis(envelope: [Vec3, Vec3, Vec3, Vec3], lenient = false): FrameBasis {
  ...
  if (residual > COPLANARITY_TOLERANCE_MM) {
    ...
    if (lenient) console.warn(msg); else throw new Error(msg);
  }
...
export function projectToFrameLocal(p: Vec3, basis: FrameBasis): Vec2 {
 ...
 */
export function transformationMatrixString(basis: FrameBasis): string {
  const fmt = (n: number) => formatFiveDecimal(n);
...
function formatFiveDecimal(n: number): string {
...
function magnitude(a: Vec3): number { return Math.sqrt(a.x * a.x + a.y * a.y + a.z * a.z); }
 ...
 */
export function coerceEnvelopeToRect(vertices: Vec3[]): [Vec3, Vec3, Vec3, Vec3] | null {
  ...
  const corner = (u: number, v: number): Vec3 => add(add(V0, scale(right, u)), scale(up, v));
  return [
    corner(uMin, vMin),
    corner(uMax, vMin),
    corner(uMax, vMax),
...
export function synthesizeRfyFromPlans(
  project: ParsedProject,
  options: SynthesizePlansOptions = {},
): SynthesizePlansResult {
  ...
  for (const plan of project.plans) {
    ...
    for (const frame of plan.frames) {
      ...
      for (const stick of frame.sticks) {
        ...
        if (
          (typeof process === "undefined" || process.env?.CODEC_DISABLE_RP_BODY_SHIFT !== "1")
          && /^T\d/.test(stick.name)
          && String(stick.usage ?? "").toLowerCase() === "topplate"
        ) {
          const bodyDir = rpSlopedTplateBodyShiftDirection(frame, stick, plan.name);
          if (bodyDir !== null) {
            const lenSlope = Math.sqrt(
              (stick.end.x - stick.start.x) ** 2 +
              (stick.end.y - stick.start.y) ** 2 +
              (stick.end.z - stick.start.z) ** 2,
            );
...
function computeFrameContextOps(
  ...
  const syntheticSticks: RfyStick[] = [];
  for (const stick of frame.sticks) {
  ...
  if (syntheticSticks.length === 0) return empty;

  const syntheticFrame: RfyFrame = {
    name: frame.name,
    weight: 0,
    length: basis.width,
    height: basis.height,
 ...
 */
function mergeStickTooling(
  perStickOps: RfyToolingOp[],
  contextOps: RfyToolingOp[],
  stickUsage?: string,
): RfyToolingOp[] {
...
function buildStickXml(stick: ParsedStick, basis: FrameBasis, frameName: string, lenient: boolean): XmlNode {
 ...
 */
function buildStickElevationGraphics(name: string, startL: Vec2, endL: Vec2, thickness: number): XmlNode {
  const dx = endL.x - startL.x;
  const dy = endL.y - startL.y;
  const len2D = Math.hypot(dx, dy);
...
function buildAnchorCircles(x: number, y: number): XmlNode[] {
  const fmt = (n: number) => roundN(n, 4).toFixed(4);
  return [
    {
 ...
 */
function buildData3dStub(stick: ParsedStick, startL: Vec2, endL: Vec2, thickness: number): XmlNode {
  const dx = endL.x - startL.x;
  const dy = endL.y - startL.y;
  const len2D = Math.hypot(dx, dy);
  const safeLen = len2D > 1e-9 ? len2D : 1;
...
function buildProfileNode(stick: ParsedStick): XmlNode {
 ...
 */
function buildFramePlanGraphicsWorld(name: string, basis: FrameBasis, wallThickness: number): XmlNode {
  const fmt = (n: number) => roundN(n, 4).toFixed(4);
  ...
  const textPos = subtract(
    add(origin, scale(right, width / 2)),
    scale(normal, wallThickness / 2),
  );
 ...
 */
function buildFrameElevationGraphicsLocal(name: string, basis: FrameBasis): XmlNode {
  const fmt = (n: number) => roundN(n, 4).toFixed(4);
  return {
    "elevation-graphics": [
      {
...
function inferStickType(usage: string): "stud" | "plate" {
  // Detailer schema: "plate" = top/bottom plates only. Nogs/sills/braces/studs are "stud".
  const u = usage.toLowerCase();
  if (u === "topplate" || u === "bottomplate" || u === "headplate" || u === "head") return "plate";
  return "stud";
}

function computeFrameWeight(sticks: ParsedStick[]): number {
  let total = 0;
