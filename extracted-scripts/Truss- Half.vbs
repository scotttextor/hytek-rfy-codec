...
'******************************************************************************
'
'   Basic Half Truss script
'
'   11 Dec 2003               Created
'   22 Sep 2005   N.Penny     Removed Boxing on Extension and Eave
'   02 Sep 2010   J.Burns     Changed Dimensioning to use CAD dimensions
'   04 Nov 2010   J.Burns     Modified to support Back to Back trusses
'
'******************************************************************************
...
Class TBuilder
  ...
  Public Sub PlaceHoweWebs
	...
	.ClipRef = drMID
        ...
        While L > Max_Web_Spacing + Web2Stud
  ...
  Public Sub PlaceFanWebs
	...
	.ClipRef = drDIMPLE
        A = .PlaceFrameObject(fotWeb, .Intersection(BCAxis, X1), .Intersection(LCAxis, X2), NOT_FLIPPED, stStud)
        L = Span - (Max_Web_Spacing * 3)

        While L > ((Max_Web_Spacing + Web2Stud) * 2)
          X2 = .Offset(X2, Web2Stud)
	  .ClipRef = drMID
          A = .PlaceFrameObject(fotWeb, .Intersection(BCAxis, X2), .Intersection(LCAxis, X2), NOT_FLIPPED, stStud)
          X1 = .Offset(X2, Web2Stud)
  ...
  Public Sub dimensionframe
    ...
    With CAD
      ...
      If eavedrop > 0 Then
        bottomoffset = -(eavedrop + dimensionoffset) + Height_At_Wall
      Else
        bottomoffset = -80
      End If
      ...
      If Extension > 0 Then 
        .PlaceDimension "0,0", "@" & Tan(MetricPitch * Pi / 180) * (Span + Extension) + Height_At_Wall - YY & "<90", -(iDimensionFirstSpace + (iDimensionCount_Left * iDimensionSpacing)) & ",0",1,-2,iDimensionFontSize,"V"
        iDimensionCount_Left = iDimensionCount_Left + 1
      End If       
      ...
      .PlaceLabel TextPitch, 0 & "," & Tan(MetricPitch * Pi / 180) * (Span + Extension) + Height_At_Wall - YY + iDimensionFirstSpace, iDimensionFontSize, 0
    End With
  End Sub
  ...
  Private Sub ReinforcePlate(ID)
    CAD.Reinforce ID, 0, Eave / Cos(MetricPitch * Pi / 180) + 150, False
  End Sub
  ...
  Private Sub Class_Initialize()
    ...
    Auto_Dimension = True
    dimensionoffset = 80 'This is the offset from an edge for a dimension
  End Sub
