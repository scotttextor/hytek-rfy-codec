...
export function computeTb2bWebPositions(
  ...
  for (let i = 0; i < sticks.length; i++) {
    for (let j = i + 1; j < sticks.length; j++) {
      ...
      const bOverride = (bIsChord && sA.usage === "web" && isWebPerpish)
        ...
        : undefined;
      // Horizontal H-header detection for chord-side correction (Agent W2, 2026-05-11).
      // When a topchord H-stick (name starts with H, zSpan<5) crosses a PAR diagonal web
      // (|dot|>0.5), the standard -CHORD_HALF_DEPTH*dot/2 formula under-corrects by
      // ~20-45mm. Empirically correct formula: -WEB_VS_RAIL_OFFSET*|dot|/sin
      // (always shifts toward lower arc position, same sign for forward/reverse webs).
      // Verified HG260044 PK1 TT1-1/H5(W7), TT3-1/H6(W16); PK2 TT18-1/H4(W7/W9/W10).
      // For PERP crossings (|dot|<0.5): correction=0 remains correct (already matched).
      const H_STICK_SIN_THRESHOLD = 0.1;
      const aIsHorizH = sA.usage === "topchord"
        && /^H\d/.test(sA.name)
        && Math.abs(sA.end3D.z - sA.start3D.z) < 5;
      const bIsHorizH = sB.usage === "topchord"
        && /^H\d/.test(sB.name)
      ...
      if (aIsChord) {
        ...
        } else if (aIsHorizH && sB.usage === "web" && absDot > 0.5) {
          // H-stick chord-side correction for PAR web: -WEB_VS_RAIL_OFFSET*|dot|/sin
          const sinVal = Math.sqrt(Math.max(0, 1 - dot * dot));
          correction = sinVal > H_STICK_SIN_THRESHOLD ? -WEB_VS_RAIL_OFFSET * absDot / sinVal : 0;
        } else {
      ...
      if (bIsChord) {
        ...
        } else if (bIsHorizH && sA.usage === "web" && absDot > 0.5) {
          // H-stick chord-side correction for PAR web: -WEB_VS_RAIL_OFFSET*|dot|/sin
          const sinVal = Math.sqrt(Math.max(0, 1 - dot * dot));
          correction = sinVal > H_STICK_SIN_THRESHOLD ? -WEB_VS_RAIL_OFFSET * absDot / sinVal : 0;
        } else {
  ...
  for (const [chordKey, crossings] of chordWebCrossings) {
    ...
    // Horizontal H-header bypass (Agent W2, 2026-05-11): horizontal H-sticks
    // (topchord, zSpan<5, name starts with H) receive both PERP and PAR web
    // crossings — Detailer does NOT suppress the PAR diagonal-web bolt near a
    // PERP vertical-web bolt on these header sticks. Verified HG260044 PK1
    // TT1-1/H5, TT2-1/H5, TTI1-1/H7; PK2 TT18-1/H4.
    const isHorizHstick = chordStick.usage === "topchord"
      && /^H\d/.test(chordStick.name)
      && Math.abs(chordStick.end3D.z - chordStick.start3D.z) < 5;
...
export function simplifyTb2bTrussFrame(
  ...
  const t7ForceReverseStickKeys = new Set<string>();
  if (/^TT/.test(frame.name)) {
    ...
    for (let i = 0; i < frame.sticks.length; i++) {
      ...
      if (!/^B\d/.test(stick.name)) continue;
      const meta = metaSticks[i]!;
      if (meta.usage !== "bottomchord") continue;
      if (meta.flipped) continue;
      const zSpan = Math.abs(meta.end3D.z - meta.start3D.z);
      if (zSpan >= 5) continue;
      if (meta.start3D.y >= meta.end3D.y) continue;
  ...
  const t9ReverseWebStickKeys = new Set<string>();
  ...
  // add the flipped=true twin's key to `forceReverseStickKeys`. The XOR with
  ...
  // the same bolt positions. Verified HG260044 PK1 TT1-1/TT4-1 T3.
  {
    ...
    for (let i = 0; i < frame.sticks.length; i++) {
      ...
      const meta = metaSticks[i]!;
      if (meta.usage !== "topchord") continue;
      if (!stick.flipped) continue;
      const zSpan = Math.abs(meta.end3D.z - meta.start3D.z);
      if (zSpan <= 5) continue; // horizontal stubs don't trigger needsArcReversal
      for (let k = 0; k < frame.sticks.length; k++) {
        if (k === i) continue;
        const o = frame.sticks[k]!;
        if (o.flipped) continue;
        if (!/^T\d/.test(o.name)) continue;
        if (/\(Box\d+\)/.test(o.name)) continue;
        const om = metaSticks[k]!;
        if (om.usage !== "topchord") continue;
  ...
  for (let stickIdx = 0; stickIdx < frame.sticks.length; stickIdx++) {
    ...
    if (isSlopedB) {
      ...
      if (slopeAngleDeg > 5) {
        ...
        for (let k = 0; k < frame.sticks.length; k++) {
          ...
          const om = metaSticks[k]!;
          const oZSpan = Math.abs(om.end3D.z - om.start3D.z);
          if (oZSpan <= 5) continue;  // peer must also be sloped
          const oLen = Math.hypot(
            om.end3D.y - om.start3D.y,
            om.end3D.z - om.start3D.z,
          );
        ...
        if (hasSlopedPeer) {
          ...
          if (totalTrim > 0.001) {
            ...
            stick.tooling = stick.tooling.flatMap((op): RfyToolingOp[] => {
              if (op.kind === "point") {
                const newPos = op.pos - trimAtOutputStart;
                if (newPos < -0.5 || newPos > newL + 0.5) return [];
              ...
              if (op.kind === "spanned") {
                ...
                if (newEnd < 0 || newStart > newL) return [];
    ...
    if (isTChordCap && meta3D) {
      ...
      // B2B twin suppression (Agent W2, 2026-05-11): if this T-chord's key is in
      // t7ForceReverseStickKeys as a B2B twin, suppress the arc-reversal so this
      // flipped=true twin emits the same apex bolt position as its non-flipped partner.
      const isTwinFlipped = t7ForceReverseStickKeys.has(stKey);
      const reverseForApex = isTwinFlipped ? false : chordArcReversal(meta3D);
    ...
    if (isTHeelCap_T5 && meta3D) {
      ...
      // B2B twin suppression: flipped=true twins use same apex direction as non-flipped partner.
      const isTwinFlipped_T5 = t7ForceReverseStickKeys.has(stKey);
      const reverse_T5 = isTwinFlipped_T5 ? false : chordArcReversal(meta3D);
      ...
      for (let k = 0; k < frame.sticks.length; k++) {
        ...
        if (dStart < HEEL_SHARE_TOL_T5 || dEnd < HEEL_SHARE_TOL_T5) {
          ...
          if (oLen > heelTipLen_T5) heelTipLen_T5 = oLen;
    ...
    if (isHHeader_T5 && isTTFrame) {
      ...
      stick.tooling = stick.tooling.filter((op): boolean => {
        ...
        if (!inEndZone) return true;
        for (const cap of headerCapPositions_T5) {
          if (Math.abs(op.pos - cap) < HEADER_KEEP_TOL_T5) return true;
        }
        return false;
      });
    }
