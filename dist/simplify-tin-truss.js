...
function tinPcDiagonalLengthExtension(angleFromVerticalDeg) {
    const a = Math.max(0, angleFromVerticalDeg);
    if (a >= 20)
        return 0;
    const rad = (a * Math.PI) / 180;
...
function extendTinPcDiagonalWLength(stick) {
    if ((stick.usage ?? "").toLowerCase() !== "web")
        return 0;
    if (!/^W\d/.test(stick.name))
        return 0;
    ...
    if (!extendStickEnd(stick, lengthShift))
        return 0;
...
function isTsnTrussFrame(frame) {
    if (!/^(TS|TN)\d+-\d+$/.test(frame.name))
        return false;
...
function findTsnChordCrossings(chord, webs) {
    const crossings = [];
    for (const w of webs) {
        const ps = projectPointOnSegment(w.start, chord.start, chord.end);
        const pe = projectPointOnSegment(w.end, chord.start, chord.end);
...
function groupTsnPanelPairs(crossings) {
    const out = [];
    const used = new Set();
    for (let i = 0; i < crossings.length; i++) {
        if (used.has(i))
        ...
        for (let j = i + 1; j < crossings.length; j++) {
            if (used.has(j))
                continue;
            if (crossings[j].tin - a.tin > TSN_PANELPOINT_MAX_PAIR_DELTA_MM)
                break;
            pi = j;
            break;
        }
...
function tsnChordEaveSign(chord) {
    const dz = chord.end.z - chord.start.z;
    if (Math.abs(dz) < TSN_SLOPED_CHORD_MIN_DZ_MM)
        return 0; // flat — not handled
...
function emitTsnTopChordPanelPattern(chord, webs) {
    const eaveSign = tsnChordEaveSign(chord);
    if (eaveSign === 0)
        return 0; // flat chord — skip
 ...
 *  parity scoring. */
function emitTsnLipNotches(chord, pairs, driftAlongChord, eaveSign) {
    if (eaveSign !== -1)
        return 0; // ascending only — see fn doc
    const chordLen = Math.hypot(chord.end.x - chord.start.x, chord.end.y - chord.start.y, chord.end.z - chord.start.z);
...
function emitTsnPanelPatternsForFrame(frame) {
    if (!isTsnTrussFrame(frame))
        return 0;
    const webs = frame.sticks.filter(s => /^W\d/.test(s.name) && (s.usage ?? "").toLowerCase() === "web");
    if (webs.length === 0)
        return 0;
 ...
 *  trusses. */
function tinPcDiagonalCrossingShift(angleFromVerticalRad) {
...
function tinPcHorizontalElementZ(stick) {
...
function tinPcWebExtendedLength(stick) {
    const dx = stick.end.x - stick.start.x;
    const dy = stick.end.y - stick.start.y;
    const dz = stick.end.z - stick.start.z;
    return Math.sqrt(dx * dx + dy * dy + dz * dz);
...
function tinPcCrossingDistFromStart(stick, zPlane) {
    const dz = stick.end.z - stick.start.z;
    if (Math.abs(dz) < 0.01)
        return null;
...
function tinPcHasNearbyDimple(stick, pos) {
    for (const op of stick.tooling) {
        if (op.kind !== "point")
            continue;
        if (op.type !== "InnerDimple")
            continue;
        if (Math.abs(op.pos - pos) < TIN_PC_NEARBY_OP_TOL_MM)
            return true;
    }
    return false;
}
...
function emitTinPcWebChordCrossings(frame) {
    ...
    for (const h of horizontalElements) {
        if (uniqZ.some(z => Math.abs(z - h.z) < 30))
            continue;
        uniqZ.push(h.z);
    }
    let emitted = 0;
    for (const stick of frame.sticks) {
        ...
        const swageHalfSpan = TIN_PC_WEB_CROSSING_BASE_HALF_SPAN_MM +
        ...
        for (const zPlane of uniqZ) {
            ...
            if (Math.abs(zPlane - stick.start.z) < 30)
                continue;
            if (Math.abs(zPlane - stick.end.z) < 30)
                continue;
            ...
            if (pos < TIN_PC_WEB_CROSSING_MIN_EDGE_MM)
                continue;
            if (pos > stickLen - TIN_PC_WEB_CROSSING_MIN_EDGE_MM)
                continue;
            if (tinPcHasNearbyDimple(stick, pos))
                continue;
            stick.tooling.push({
                kind: "point",
                type: "InnerDimple",
