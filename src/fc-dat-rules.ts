...
function makeKey(type: TrussType, side: GeometrySide, profile: ProfileIndex): string {
...
const RULES: Record<string, TrussGeometryRule> = {
...
export function getTrussGeometry(
  type: TrussType,
  side: GeometrySide,
  profile: ProfileIndex,
): TrussGeometryRule | undefined {
  return RULES[makeKey(type, side, profile)];
}

/** Convenience: resolve the C-side TRU profile by HYTEK profile-web (mm). */
export function getCSideTruProfileForWeb(
  webMm: number,
  centreHole = false,
  linear = false,
): ProfileIndex | null {
  if (linear && webMm === 89) return 5;
  if (centreHole) {
    if (webMm === 89 || webMm === 70) return webMm === 89 ? 3 : 4;
    return null;
  }
  if (webMm === 89 || webMm === 90) return 0;
  if (webMm === 75) return 1;
  if (webMm === 70) return 2;
  return null;
}
...
export function getAllRules(): Readonly<Record<string, TrussGeometryRule>> {
  return RULES;
}
