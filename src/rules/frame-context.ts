 ...
 */
export function generateFrameContextOps(
  frame: RfyFrame,
  setup?: MachineSetup,
  ...
  // ---------------------------------------------------------------------
  const actionDefsOpsByStick = new Map<string, RfyToolingOp[]>();
  ...
  const studs = layout.filter(sb => {
    if (!STUD_ROLES.has(sb.role)) return false;
  ...
  const wallWBraces = layout.filter(sb =>
    sb.role === "W" &&
    String(sb.stick.usage ?? "").toLowerCase() !== "web"
  );
  ...
  for (const plate of plates) {
    ...
    const planNameForRpTrim = (frame as { planName?: string }).planName ?? "";
    ...
    for (const stud of allCrossingStuds) {
      ...
      if (crossingX < plateLongLo + 50) continue;
      if (crossingX > plateLongHi - 50) continue;
    ...
    interface WebCrossing {
      ...
      localPosHi: number;
      offset: number;
      centerLocalPos: number;
    }
    const webCrossings: WebCrossing[] = [];
    ...
    for (const web of allWebsForCrossing) {
      ...
      function intersectAtY(p1: RfyPoint, p2: RfyPoint, atY: number): number | null {
        const dy = p2.y - p1.y;
      ...
      if (sinTheta < 0.1) continue;  // near-horizontal edge — degenerate
    ...
    interface NotchCluster {
      startPos: number;
      endPos: number;
      centers: number[];  // for InnerDimples
    }
    ...
    for (const c of clusters) {
      if (c.startPos < 0) c.startPos = 0;
      if (c.endPos > plate.stick.length) c.endPos = plate.stick.length;
      const quantizedCenter = Math.round((c.centers[0]!) / 30) * 30;
      if (seenPositions.has(quantizedCenter)) continue;
  ...
  function studHasLipNeighbor(stud: StickWithBox): boolean {
    const sign = stud.stick.flipped ? +1 : -1;
    const cx = stud.box.cx;
    for (const other of studs) {
      if (other.stick.name === stud.stick.name) continue;
      const dx = other.box.cx - cx;
      if (sign > 0 && dx >= 5 && dx <= 45) return true;
      if (sign < 0 && dx <= -5 && dx >= -45) return true;
    }
    return false;
  }
  ...
  for (const p of plates) {
    if (p.box.xMin < wallXMin) wallXMin = p.box.xMin;
    if (p.box.xMax > wallXMax) wallXMax = p.box.xMax;
  }
  ...
  for (const header of headers) {
    ...
    const planNameForHeader = (frame as { planName?: string }).planName ?? "";
  ...
  for (const nog of nogs) {
    ...
    const nogCrossings: { localPos: number; lipSpan: number }[] = [];
    for (const stud of studs) {
    ...
    const ranges: { startPos: number; endPos: number; centers: number[] }[] = [];
 ...
 */
export function mergeActionDefsOps(legacyOps: RfyToolingOp[], addOps: RfyToolingOp[]): void {
  const TOL = 0.15;
  function key(op: RfyToolingOp): { type: string; kind: string; a: number; b: number } {
