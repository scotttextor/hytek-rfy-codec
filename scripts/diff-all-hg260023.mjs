#!/usr/bin/env node
/**
 * Run diff-vs-detailer.mjs against every HG260023 plan and produce a unified
 * baseline report. Reference data lives on Y: drive (no OneDrive cache yet).
 *
 * Modeled on diff-all-hg260001.mjs (same Y-drive layout). Adds an explicit
 * Y-drive-mounted guard with a clear error message — previous agents have
 * stalled when Y: was disconnected mid-run.
 *
 * Output: scripts/baselines/hg260023/baseline.json + .md (per-plan +
 * per-tool aggregates), with per-plan raw diffs in scripts/baselines/hg260023/raw/.
 */
...
const OUT_DIR = "scripts/baselines/hg260023";

// Y-drive guard: fail fast and clearly, don't let spawnSync errors muddy the report.
...
// Same suffix-based XML→RFY matcher as HG260001/HG260044 (which both ship the
// same XML→multi-PK split pattern). One TB2B XML feeds 5 PK RFYs (PK7..PK11);
// they all share the suffix "GF-TB2B-70.075".
...
// Aggregate: per-tool, plus per-frame-type (CP/LBW/MH/NLBW/RP/TB2B/TIN).
function frameTypeOf(rfy) {
  const m = rfy.match(/GF-([A-Za-z0-9]+)-\d+\.\d+\.rfy$/i);
  return m ? m[1].toUpperCase() : "?";
}
const aggMissing = {}, aggExtras = {};
const byFrameType = {}; // { LBW: { ref, matched, missing, extras }, ... }
...
const lines = [];
lines.push("# HG260023 Baseline — Per-Plan Op-Level Diff vs Detailer");
lines.push("");
