...
import { getActionSection, type ActionSlot } from "./action-defs.js";
import {
  evalConditions,
  packEdgeMask,
...
import { emitActions, type EmitContext } from "./action-emit.js";
import type { StickWithBox } from "./frame-context.js";
import type { MachineSetup, SectionSetup } from "../machine-setups.js";
...
export function isActionDefsPassEnabled(): boolean {
 ...
 */
function deriveStickProps(sb: StickWithBox, setup?: MachineSetup): StickProps {
  ...
  let sectionSetup: SectionSetup | undefined;
...
// ---------------------------------------------------------------------------
// Crossings detection — extended to cover truss panel-points
// ---------------------------------------------------------------------------
...
export type TIntersectionType =
  | "ww_inner_edge"  // web-to-web (panel-point dimples on chords)
  | "ew_inner_edge"  // end-to-web (brace end into chord web)
  | "ll_inner_edge"  // lip-to-lip
  | "lw_inner_edge"  // lip-on-web
  | "wl_inner_edge"  // web-on-lip
  | "t_bchord"       // top stick → bottom-chord intersection
  | "b_tchord"       // bottom stick → top-chord intersection
  | "t_tchord"       // top-stick → top-chord intersection
  | "explicit_tool"; // explicit-tool sentinel

interface Crossing {
  /** Connector stick (the one we're emitting ops onto). */
  connector: StickWithBox;
  /** Connectee stick (the partner in the joint). */
  connectee: StickWithBox;
  /** Position of the crossing on the connector (mm from start). */
  intersectionPos: number;
  /** EdgeFlags for the slot index — packed into 0..15. */
  edges: EdgeFlags;
  /** Web-angle in degrees (0..180). 90 = perpendicular. */
  webAngleDeg?: number;
  /** Detailer intersection-type tag — drives chord-flag assignment. */
...
function isTrussChordUsage(u: string): boolean {
  const x = u.toLowerCase();
  return x === "topchord" || x === "bottomchord";
}

/** Recognise truss web usage strings. */
function isTrussWebUsage(u: string): boolean {
 ...
 */
function getCenterlineEndpoints(sb: StickWithBox): { sx: number; sy: number; ex: number; ey: number } | null {
...
function intersectSegments(
  ...
  b: { sx: number; sy: number; ex: number; ey: number }
  ...
  const SLACK = 0.05;
  if (tA < -SLACK || tA > 1 + SLACK) return null;
  if (tB < -SLACK || tB > 1 + SLACK) return null;
  return {
    x: a.sx + tA * ax,
    y: a.sy + tA * ay,
    tA, tB,
...
function angleBetween(a: { sx: number; sy: number; ex: number; ey: number },
                      b: { sx: number; sy: number; ex: number; ey: number }): number {
  const ax = a.ex - a.sx, ay = a.ey - a.sy;
  const bx = b.ex - b.sx, by = b.ey - b.sy;
...
function projectToStickLocal(
  cl: { sx: number; sy: number; ex: number; ey: number },
  px: number, py: number
): number {
...
function computeEdgeMask(
  ...
  const edges: EdgeFlags = { ll: false, lw: false, wl: false, ww: true };
  if (isEndPartner && !isEndConnector) edges.ew = true;
  if (isEndConnector && !isEndPartner) edges.el = true;
  return edges;
}
...
function classifyIntersectionType(
  connector: StickWithBox,
  connectee: StickWithBox,
  tA: number, tB: number,
): TIntersectionType {
  ...
  if (cnIsChord && ceIsChord) {
    if (cnUsage === "topchord" && ceUsage === "bottomchord") return "t_bchord";
    if (cnUsage === "bottomchord" && ceUsage === "topchord") return "b_tchord";
    if (cnUsage === "topchord" && ceUsage === "topchord") return "t_tchord";
  ...
  const isEndPartner = tB < 0.05 || tB > 0.95;
  if (isEndPartner) return "ew_inner_edge";
 ...
 */
function findCrossings(layout: StickWithBox[]): Crossing[] {
  const out: Crossing[] = [];
  const seen = new Set<string>();  // dedupe (connector, connectee) pairs

  // -------------------------------------------------------------------
  // BRANCH 1 — wall plate × stud (orthogonal bbox-overlap, legacy)
  // -------------------------------------------------------------------
  const connectorsHorizontal = layout.filter(
    (sb) => CONNECTOR_ROLES.has(sb.role) && sb.horizontal
  );
  const partnersVertical = layout.filter(
    (sb) => PARTNER_ROLES.has(sb.role) && !sb.horizontal
  );
  for (const connector of connectorsHorizontal) {
    for (const partner of partnersVertical) {
      if (partner.box.cx < connector.box.xMin) continue;
      if (partner.box.cx > connector.box.xMax) continue;
      if (partner.box.yMax < connector.box.yMin) continue;
      if (partner.box.yMin > connector.box.yMax) continue;
      const intersectionPos = partner.box.cx - connector.box.xMin;
      if (intersectionPos < 50) continue;
      if (intersectionPos > connector.stick.length - 50) continue;
  ...
  for (const connector of trussSticks) {
    ...
    for (const partner of trussSticks) {
      ...
      if (!partnerIsParticipant) continue;
...
function pickAlternative(slot: ActionSlot, condCtx: ConditionContext) {
  for (const alt of slot.alternatives) {
    if (evalConditions(alt.conditions, condCtx)) return alt;
  }
  return undefined;
}
 ...
 */
function getSuppressedSet(): Set<JointClassification> {
  const out = new Set<JointClassification>();
 ...
 */
export function runActionDefsPass(
  layout: StickWithBox[],
  config: ActionDefsPassConfig,
): Map<string, ActionDefsPassInfo> {
  const out = new Map<string, ActionDefsPassInfo>();
  ...
  const flags: JointFlags = config.frameFlags ?? (config.planName
  ...
  const partnerCountByConnector = new Map<string, number>();
  ...
  const opsByStick = new Map<string, RfyToolingOp[]>();
  const tracesByStick = new Map<string, string[]>();
  const classByStick = new Map<string, JointClassification>();
  ...
  for (const cr of crossings) {
    ...
    if (SUPPRESSED_CLASSIFICATIONS.has(className)) continue;
    ...
    if (emit.ops.length === 0) continue;

    const name = cr.connector.stick.name;
    if (!opsByStick.has(name)) opsByStick.set(name, []);
    opsByStick.get(name)!.push(...emit.ops);
    if (!tracesByStick.has(name)) tracesByStick.set(name, []);
    if (emit.trace) tracesByStick.get(name)!.push(emit.trace);
...
export function emptyActionDefsPass(layout: StickWithBox[]): Map<string, ActionDefsPassInfo> {
  const out = new Map<string, ActionDefsPassInfo>();
  for (const sb of layout) {
    out.set(sb.stick.name, { handled: false, ops: [] });
  }
