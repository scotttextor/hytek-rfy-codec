...
// TB2B post-decode rewriter removed 2026-05-05 (Agent O):
// computeTB2BWebPositions and related helpers were migrated to
// src/simplify-tb2b-truss.ts (export computeTb2bWebPositions).
// The harness no longer needs to compute TB2B Web@pt positions itself —
// the codec's production output already includes them.
  ...
  for (const p of root.plan ?? []) {
    ...
    for (const f of p.frame ?? []) {
      ...
      for (const s of f.stick ?? []) {
        ...
        // [Agent V, 2026-05-05] InnerService z-line consumption (Agent S's
        // 2026-05-05 patch) was MIGRATED INTO src/simplify-wall-service.ts.
        // The harness now populates frame.serviceActions (see frames.push
        // below) and synthesizeRfyFromPlans calls
        // simplifyWallServiceInProject — production callers
        // (hytek-rfy-tools) get the same fix.
        // See docs/simplify-wall-service-design.md for the migration audit.
...
for (const plan of ourDoc.project.plans) {
  ...
  for (const frame of plan.frames) {
    // TB2B post-decode rewrite removed 2026-05-05 (Agent O):
    // Logic now lives in src/simplify-tb2b-truss.ts and runs as a post-pass
    // inside synthesizeRfyFromPlans, so the codec's production .rfy output
    // matches the diff-harness numbers without after-the-fact patching.
    // See docs/tb2b-patch-audit.md for the migration audit.
