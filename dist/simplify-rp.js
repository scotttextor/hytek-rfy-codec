...
function addBothEndChamfers(stick, opts = {}) {
    let added = 0;
...
function isRpStackedRakeLongTPlate(frame, stick) {
    if (!isTplate(stick))
        return false;
    ...
    if (stickLen < RP_STACKED_RAKE_SHORT_MAX_MM) {
        ...
        if (Math.abs(stick.start.z - peakZ) < HORIZONTAL_BOTTOM_TOL_MM)
            return true;
        return false;
    }
    if (stickLen < RP_STACKED_RAKE_LONG_MIN_MM)
        return false;
    ...
    for (const t of tPlates) {
        if (t === stick)
            continue;
        const dStart = Math.sqrt((stick.start.x - t.start.x) ** 2 +
            (stick.start.y - t.start.y) ** 2);
        const dEnd = Math.sqrt((stick.start.x - t.end.x) ** 2 +
            (stick.start.y - t.end.y) ** 2);
        if (dStart < RP_STACKED_RAKE_T_NEAR_TOL_MM)
            return false;
        if (dEnd < RP_STACKED_RAKE_T_NEAR_TOL_MM)
            return false;
    }
    ...
    if (sisterLen < RP_STACKED_RAKE_SHORT_MAX_MM && bPlates.length > 0) {
        for (const b of bPlates) {
            if (Math.abs(stick.start.z - b.start.z) < RP_STACKED_RAKE_BZ_MATCH_TOL_MM)
                return true;
            if (Math.abs(stick.start.z - b.end.z) < RP_STACKED_RAKE_BZ_MATCH_TOL_MM)
                return true;
        }
    }
...
function rewriteRpTplateStartCapToWall(stick) {
    ...
    stick.tooling.push({ kind: "spanned", type: "Swage", startPos: 0, endPos: STD_END_SPAN_MM }, { kind: "point", type: "InnerDimple", pos: STD_DIMPLE_OFFSET_MM });
}
...
function detectRpBplateEaveOverhangWithBranch(stick, frame) {
    ...
    for (const t of frame.sticks) {
        ...
        for (const pt of [t.start, t.end]) {
            ...
            if (-proj > maxOverhang)
                maxOverhang = -proj;
            if (proj - len > maxOverhang)
                maxOverhang = proj - len;
        }
    }
    return {
        eaveEnd: eave,
...
function detectRpBplateEaveOverhang(stick, frame) {
    const dx = stick.end.x - stick.start.x;
    const dy = stick.end.y - stick.start.y;
    const dz = stick.end.z - stick.start.z;
    const len = Math.sqrt(dx * dx + dy * dy + dz * dz);
    ...
    for (const t of frame.sticks) {
        ...
        for (const pt of tEndpoints) {
            const proj = (pt.x - stick.start.x) * ux + (pt.y - stick.start.y) * uy + (pt.z - stick.start.z) * uz;
            if (proj < -maxOverhangStart)
                maxOverhangStart = -proj;
            if (proj - len > maxOverhangEnd)
                maxOverhangEnd = proj - len;
        }
    }
...
function stripRpBplateWallCapStart(tooling) {
    let removed = 0;
    for (let i = tooling.length - 1; i >= 0; i--) {
        const op = tooling[i];
        if (op.kind === "spanned"
...
function stripRpBplateWallCapEnd(tooling, stickLen) {
    let removed = 0;
    for (let i = tooling.length - 1; i >= 0; i--) {
        const op = tooling[i];
        if (op.kind === "spanned"
 ...
 */
function applyRpBplateEaveExtension(stick, eaveEnd, capType = "Swage") {
    const dx = stick.end.x - stick.start.x;
    const dy = stick.end.y - stick.start.y;
    const dz = stick.end.z - stick.start.z;
    const len = Math.sqrt(dx * dx + dy * dy + dz * dz);
    ...
    if (eaveEnd === "start") {
        ...
        stick.tooling.push({ kind: "spanned", type: capType, startPos: 0, endPos: RP_BPLATE_CHORD_CAP_SWAGE_SPAN_MM }, { kind: "point", type: "InnerDimple", pos: RP_BPLATE_CHORD_CAP_DIMPLE_OFFSET_MM });
    }
    else {
...
function rewriteRpBplateNonEaveWallCapType(stick, eaveEnd) {
    const dx = stick.end.x - stick.start.x;
    const dy = stick.end.y - stick.start.y;
    const dz = stick.end.z - stick.start.z;
    const len = Math.sqrt(dx * dx + dy * dy + dz * dz);
...
export function simplifyRpFrame(frame) {
    ...
    for (const stick of frame.sticks) {
        ...
        if (detection === null)
            continue;
        const capType = detection.branch === "A" ? "Swage" : "LipNotch";
...
function frameSingleTplateRakeDirection(frame) {
    let tCount = 0;
    let nCount = 0;
    let t1 = null;
    let maxStudLen = 0;
    ...
    if (maxStudLen / len > 1.0)
        return null;
    if (dz >= RP_RAKE_T_DZ_MIN_MM)
        return "asc";
    if (dz <= -RP_RAKE_T_DZ_MIN_MM)
        return "desc";
    return null;
}
export function applyRpSingleTplateRakeCap(tooling, stickLen, direction) {
    let stripped = 0;
    let added = 0;
    let shifted = 0;
...
export function rpRakeDirectionForFrame(frame, planName) {
    if (!isRpPlanName(planName))
        return null;
...
export function applyRpSlopedTplateBodyShift(tooling, stickLen, direction) {
    const bodyLo = 150;
 ...
 */
export function rpSlopedTplateBodyShiftDirection(frame, stick, planName) {
    if (!isRpPlanName(planName))
        return null;
    if (!isTplate(stick))
        return null;
    if (String(stick.usage ?? "").toLowerCase() !== "topplate")
        return null;
    const dz = stick.end.z - stick.start.z;
    if (Math.abs(dz) < RP_RAKE_T_DZ_MIN_MM)
        return null;
