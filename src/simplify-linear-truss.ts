...
function processFrame(
  ...
  const rawJunctionsPerStick = new Map<string, RawJunction[]>();
  ...
  const finalJunctionsPerStick = new Map<string, Junction[]>();
  ...
  let totalNewBolts = 0;
  const writtenSticks = new Set<string>();
  ...
  if (modifiedSticks > 0) {
    ...
    const sticksJunctions: StickJunctions[] = [];
    for (const stick of parsed.sticks) {
      if (!writtenSticks.has(stick.name)) continue;
...
function pushJunction(
  ...
  const arr = map.get(key);
  const entry: RawJunction = { pos, mate, mateUsage };
...
export function dedupApexWithMates(
  raw: readonly RawJunction[],
  apexCollisionMm: number,
): Junction[] {
  const sorted = [...raw].sort((a, b) => a.pos - b.pos);
  const out: Array<{ posMm: number; mates: JunctionMate[] }> = [];
